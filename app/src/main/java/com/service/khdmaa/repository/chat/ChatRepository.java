package com.service.khdmaa.repository.chat;
...
//    }
    ...
    public LiveData<Resource<Boolean>> deleteMessagesToFirebaseByChatHeadId(Message message) {
        ...
        if(message != null && message.sessionId != null && message.sessionId.trim() != "" && message.id != null && message.id.trim() != "") {

            DatabaseReference pushedPostRef = databasereference.child(Config.MESSAGE).child(message.sessionId).child(message.id);
            pushedPostRef.child("message").setValue("deleted").addOnCompleteListener(task -> {
...
//                        if(snapshot.getValue() == Message.class) {
                        try {
                            if(snapshot != null) {
