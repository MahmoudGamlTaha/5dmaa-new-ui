package com.service.khdmaa.ui.item.itemlocation;
...
import com.service.khdmaa.databinding.FragmentItemLocationBinding;
 ...
 */
public class ItemEntryLocationFragment extends PSFragment implements DataBoundListAdapter.DiffUtilDispatchedInterface{
        ...
        try {
            if (getActivity() != null) {
                if (getActivity().getIntent().getExtras() != null) {
                    ...
                    itemViewModel.locationTownshipId = getActivity().getIntent().getStringExtra(Constants.ITEM_TOWNSHIP_TYPE_ID);
                    itemViewModel.locationTownshipName = getActivity().getIntent().getStringExtra(Constants.ITEM_TOWNSHIP_TYPE_NAME);
                    itemLocationViewModel.locationId = selected_location_id;
                    Utils.psLog(itemViewModel.itemId);
                    Utils.psLog(itemViewModel.cityId);
                }
            }
        } catch (Exception e) {
            Utils.psErrorLog("", e);
    ...
    protected void initAdapters() {
        ...
        ItemLocationAdapter nvadapter = new ItemLocationAdapter(dataBindingComponent,
                (itemLocation, id) -> {
                    ...
                    if (ItemEntryLocationFragment.this.getActivity() != null) {

                            navigationController.navigateBackToItemLocationFragment(ItemEntryLocationFragment.this.getActivity(), itemLocation.id, itemLocation.name, itemLocation.lat, itemLocation.lng);

                        ItemEntryLocationFragment.this.getActivity().finish();
                    }
                }, this.locationId);
