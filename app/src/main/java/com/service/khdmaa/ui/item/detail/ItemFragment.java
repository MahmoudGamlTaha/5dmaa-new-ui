package com.service.khdmaa.ui.item.detail;
...
import com.service.khdmaa.databinding.FragmentItemBinding;
...
import com.service.khdmaa.utils.ViewAnimationUtil;
...
public class ItemFragment extends PSFragment {
    ...
    protected void initUIAndActions() {
       ...
       //     binding.get().adView.loadAd(adRequest);
        ...
        binding.get().editButton.setOnClickListener(v -> navigationController.navigateToItemEntryActivity(getActivity(), itemViewModel.itemId, itemViewModel.locationId, itemViewModel.locationName, itemViewModel.locationTownshipId, itemViewModel.locationTownshipName));
                    ...
                    case SUCCESS:
                        ...
                        if (result.data != null) {
                            ...
                            if (result.data.inAppPurchasedEnabled.equals(Constants.ONE) &&
                                    ...
                                    result.data.offlineEnabled.equals(Constants.ZERO)
                            ) {
                    ...
                    switch (listResource.status) {
                        case LOADING:
                            ...
                            if (listResource.data != null) {
                                ...
                                bindingFollowerCount(listResource.data);
                                ...
                                itemViewModel.followerCount = listResource.data.user.followerCount;
                            }
                        ...
                        case SUCCESS:
                            ...
                            if (listResource.data != null) {
                                ...
                                itemViewModel.locationTownshipId = listResource.data.itemTownshipLocation.id;
                                itemViewModel.addedUserId = listResource.data.addedUserId;
                                itemViewModel.locationName = listResource.data.itemLocation.name;
                                itemViewModel.locationTownshipName = listResource.data.itemTownshipLocation.townshipName;
...
//        }
    ...
    private void bindingFavouriteCount(Item item) {
        String favouriteCount = item.favouriteCount;
        binding.get().likesTextView.setText(favouriteCount);

    }

    private void bindingFollowerCount(Item item) {
        String followerCount =  item.user.followerCount;
        binding.get().countTextView.setText(followerCount);
    ...
    private void showOrHide(Item item) {
        ...
        if (item != null && item.user.followerCount != null && item.user.followerCount.equals("")) {
            binding.get().countTextView.setVisibility(View.GONE);
            binding.get().countTextView.setVisibility(View.GONE);
        } else {
            binding.get().countTextView.setVisibility(View.VISIBLE);
            binding.get().imageView32.setVisibility(View.VISIBLE);
        }
